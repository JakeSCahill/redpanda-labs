...
// (test start {"id":"data-transform-flatten-go", "description": "Flatten JSON messages", "setup": "doc-tests/fetch-versions-and-rpk.json", "cleanup": "doc-tests/cleanup.json"})
...
docker compose up -d
```
// (step {"action":"runShell", "command": "docker compose up -d"})

. Set up your rpk profile:
+
```bash
rpk profile create flatten
```
// (step {"action":"runShell", "command": "rpk profile create flatten --from-profile profile.yml"})
...
// (step {"action":"runShell", "command": "rpk transform deploy --input-topic=src --output-topic=sink --var 'RP_FLATTEN_DELIM=.'"})
...
```
// (step {"action":"runShell", "command": "bash ../../setup-tests/tools/rpk-produce.sh", "args": ["'{\"message\": \"success\", \"timestamp\": 1707743943,\"iss_position\": {\"latitude\": \"-28.5723\", \"longitude\": \"-149.4612\"}}'"]})
...
```
// (step {"action":"runShell", "command": "rpk topic consume sink --num 1", "output": "/position\\.longitude/"})
+
[json, role="no-copy"]
----
{
